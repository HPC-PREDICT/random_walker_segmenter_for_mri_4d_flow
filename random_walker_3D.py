...
# calculate the intensity part here
# ==========================================
...
def _compute_dotproduct_3d(data, spacing):
    ...
    eps = 1e-6    
    dot_deep_norm = dot_deep / (dot_deep_magn + eps)
    dot_right_norm = dot_right / (dot_right_magn + eps)
    dot_down_norm = dot_down / (dot_down_magn + eps)
...
#             'function in "cg_mg" mode (see docstring).')
    ...
    # If the array has pruned zones, be sure that no isolated pixels exist between pruned zones (they could not be determined)
    if np.any(labels < 0):
    ...
    # lap_sparse X = B
    # where X[i, j] is the probability that a marker of label i arrives first at pixel j by anisotropic diffusion.
    if mode == 'cg':
    ...
    if mode == 'cg_mg':
        if not amg_loaded:
            warn("""pyamg (http://pyamg.org/)) is needed to use this mode, but is not installed. The 'cg' mode will be used instead.""")
            X = _solve_cg(lap_sparse,
                          B,
                          tol=tol,
                          return_full_prob=return_full_prob)
