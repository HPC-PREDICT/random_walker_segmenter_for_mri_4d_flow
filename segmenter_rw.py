...
mr_io_dir_path = current_dir_path[:-49] + 'hpc-predict-io/python/'
sys.path.append(mr_io_dir_path)
from mr_io import FlowMRI, SegmentedFlowMRI

# ============================   
# Set Paths of the input FlowMRI image and the output SegmentedFlowMRI segmentation
# ============================   
basepath_image = '/usr/bmicnas01/data-biwi-01/nkarani/projects/hpc_predict/data/eth_ibt/flownet/pollux/all_data/'
subnum = 7
flowmripath = basepath_image + 'v' + str(subnum) + '_R1.h5'
segmentedflowmripath = basepath_image + 'v' + str(subnum) + '_seg_rw_tmp.h5'
...
class MainWindow():
    ...
    # ============================
    # Make the image of a pre-determined size
    # ============================
    x_size_orig, y_size_orig, z_size, t_size, num_channels = flowMRI_image.shape
    flowMRI_image = utils.resize_flormri(112, 112, flowMRI_image, 5)
    ...
    markers_4d_eroded = np.zeros((flowMRI_image.shape[0], flowMRI_image.shape[1], flowMRI_image.shape[2], flowMRI_image.shape[3]))
    markers_4d_user = np.zeros((flowMRI_image.shape[0], flowMRI_image.shape[1], flowMRI_image.shape[2], flowMRI_image.shape[3]))
    ...
    def mousecallback(self,event):
        ...
        if x > 0 and y > 0 and x < (self.x_size-1)*3 and y < (self.y_size-1)*3:
            print('Coordinates of the marked point: ' + str(coord_tuple))
            if self.v.get() == 1:
    ...
    def add_markers_to_3d_list(self):
        if self.fg_list_2d or self.bg_list_2d:
            ...
            for t in self.fg_list_2d:
                print('Coordinates of the marker added to the marker list: ' + str(round(t[0] / 3)) + ', ' + str(round(t[1] / 3)))
            ...
            for tt in range(self.t_size):
                self.markers_4d_user[:, :, :, tt] = self.markers_3d
    ...
    def run_random_walker3D(self):
        ...
        for tt in range(self.t_size):
            ...
            self.markers_4d_eroded[:, :, :, tt] = eroded_labels3D
            self.markers_4d_user[:, :, :, tt] = self.markers_3d
    ...
    def run_random_walker4D3D(self):
        ...
        for zz in range(self.z_size):
            rw_labels3D = rw3D.random_walker(self.flowMRI_image[:, :, zz, :, :],
                                             self.markers_4d_eroded[:, :, zz, :],
                                             mode = 'cg_mg',
                                             return_full_prob = True,
                                             alpha = alpha_a,
                                             beta = beta_b,
            ...
            self.rw_labels[:, :, zz, :] = rw_labels3D[0, :, :, :]
    ...
    def toggle_segmentation(self):
        if self.button_display_segmentation['text'] == "View soft segmentation":
            ...
            self.button_display_segmentation.configure(text="View user markers")
            self.rw_labels_view = np.round(self.markers_4d_eroded)
            self.update_image()
            
        elif self.button_display_segmentation['text'] == "View user markers":
            self.button_display_segmentation.configure(text="View soft segmentation")
            self.rw_labels_view = np.round(self.markers_4d_user)
            self.update_image()
    ...
    def save_segmentation(self):
        ...
        segmented_flow_mri = SegmentedFlowMRI(self.flow_mri.geometry,
                                              ...
                                              utils.resize_flormri(self.x_size_orig, self.y_size_orig, self.rw_labels, 4))
