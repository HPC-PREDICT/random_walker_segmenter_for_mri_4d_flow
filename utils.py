...
def resize_flormri(nx, ny, image, dim = 5):
    ...
    if x > nx and y > ny:
        if dim == 5:
            image_resized = image[x_s:x_s + nx, y_s:y_s + ny, :, :, :]
        elif dim == 4:
            image_resized = image[x_s:x_s + nx, y_s:y_s + ny, :, :]
    else:
        if dim == 5:
            image_resized = np.zeros((nx, ny, image.shape[2], image.shape[3], image.shape[4]))
            if x <= nx and y > ny:
                image_resized[x_c:x_c + x, :, :, :, :] = image[:, y_s:y_s + ny, :, :, :]            
            elif x > nx and y <= ny:
                image_resized[:, y_c:y_c + y, :, :, :] = image[x_s:x_s + nx, :, :, :, :]
            else:
                image_resized[x_c:x_c + x, y_c:y_c + y, :, :, :] = image
                
        elif dim == 4:
            image_resized = np.zeros((nx, ny, image.shape[2], image.shape[3]))
            if x <= nx and y > ny:
                image_resized[x_c:x_c + x, :, :, :] = image[:, y_s:y_s + ny, :, :] 
            elif x > nx and y <= ny:
                image_resized[:, y_c:y_c + y, :, :] = image[x_s:x_s + nx, :, :, :]
